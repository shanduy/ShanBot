	...
	case 'hidetag':
              ...
              await client.sendMessage(from, options, text)
               break
					...
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?apikey=apivinz&url=${args[0]}`, {method: 'get'})
				...
				case 'ytmp4':
					if (args.length < 1) return reply('Donde esta la URL?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4?apikey=apivinz&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
	        ...
	        if (!isUser) return reply(mess.only.daftarB)
                reply(mess.wait)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?apikey=${play}&apikey=apivinz`)
               if (anu.error) return reply(anu.error)
